 ...
 * Format of comment
 */
function op001_comment_format( $comment, $args, $depth ) {
	$GLOBALS[ 'comment' ] = $comment;
	extract( $args, EXTR_SKIP );

	?>
<li <?php comment_class( empty( $args[ 'has_children' ] ) ? 'c-comment-item' : 'c-comment-item c-comment-item_parent' ); ?> id="comment-id-<?php comment_ID(); ?>" itemscope itemtype="http://schema.org/Comment">
	<header class="c-comment-item__header">
		...
		<div class="c-comment-item__meta">
			...
			<time class="c-comment-item__time" data-time="<?php comment_date('Y-m-d') ?>T<?php comment_time('H:iP') ?>" itemprop="datePublished"><?php comment_date() ?> <?php comment_time() ?></time>
		</div>
	</header>
 ...
 */
add_filter('comment_reply_link', 'op001_replace_reply_link_class');
function op001_replace_reply_link_class( $class ){
 ...
 */
add_filter( 'edit_comment_link', 'op001_edit_comment_link' );
function op001_edit_comment_link( $link ) {
 ...
 */
add_filter( 'comment_form_default_fields', 'op001_comment_form_fields' );
function op001_comment_form_fields( $fields ) {
 ...
 * custom comment form
 */
add_filter( 'comment_form_defaults', 'op001_comment_form' );
function op001_comment_form( $args ) {
